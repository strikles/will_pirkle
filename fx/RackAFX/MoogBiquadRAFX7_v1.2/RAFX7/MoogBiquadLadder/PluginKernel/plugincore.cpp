...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::filterFc, "fc", "Hz", controlVariableType::kDouble, 20.000000, 20480.000000, 8000.000000, taper::kVoltOctaveTaper);
	...
	piParam = new PluginParameter(controlID::filterQ, "Q", "", controlVariableType::kDouble, 1.000000, 10.000000, 8.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&filterQ, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- continuous control: Bass Boost
	piParam = new PluginParameter(controlID::bassBoost_Pct, "Bass Boost", "%", controlVariableType::kDouble, 0.000000, 100.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&bassBoost_Pct, boundVariableType::kDouble);
	addPluginParameter(piParam);

	// --- discrete control: AutoLimiter
	piParam = new PluginParameter(controlID::enableAutoLimiter, "AutoLimiter", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&enableAutoLimiter, boundVariableType::kInt);
	...
	// --- controlID::bassBoost_Pct
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483670);
	setParamAuxAttribute(controlID::bassBoost_Pct, auxAttribute);

	// --- controlID::enableAutoLimiter
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741824);
	setParamAuxAttribute(controlID::enableAutoLimiter, auxAttribute);
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	// --- reset filters
	moogFilters[0].reset(resetInfo.sampleRate);
	moogFilters[1].reset(resetInfo.sampleRate);
...
{
	MoogLPFParameters params = moogFilters[0].getParameters();
	params.fc = filterFc;
	params.Q = filterQ;
	params.bassBoost_Pct = bassBoost_Pct;
	params.enableAutoLimiter = (enableAutoLimiter == 1);

	moogFilters[0].setParameters(params);
	moogFilters[1].setParameters(params);
}
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	// --- process output (yes, I know it is verbose)
	double ynL = moogFilters[0].processAudioSample(xnL);
    ...
    // --- Stereo-In/Stereo-Out
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		// --- do right channel (yes, it is verbose)
		double xnR = processFrameInfo.audioInputFrame[1];
		double ynR = moogFilters[1].processAudioSample(xnR);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::filterFc, 7999.999512);
	setPresetParameter(preset->presetParameters, controlID::filterQ, 8.000000);
	setPresetParameter(preset->presetParameters, controlID::bassBoost_Pct, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::enableAutoLimiter, -0.000000);
	addPreset(preset);
