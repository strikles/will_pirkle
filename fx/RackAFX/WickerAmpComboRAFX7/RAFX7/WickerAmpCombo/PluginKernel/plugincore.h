...
public:
	...
	// --- C++ Effect Objects: simple declaration as this unit processes frames
	//
	WickerAmp wickerAmp;
...
private:
	...
	double lowCut_010 = 0.0;
	double bassControl_010 = 0.0;
	double midControl_010 = 0.0;
	double trebleControl_010 = 0.0;
	double masterVolume_010 = 0.0;
	double tubeCompress_010 = 0.0;
	double masterReverb_010 = 0.0;

	// --- Discrete Plugin Variables 
	int toneStackContour = 0;
	enum class toneStackContourEnum { none,normal,mid_boost };	// to compare: if(compareEnumToInt(toneStackContourEnum::none, toneStackContour)) etc... 

	int enableBright = 0;
	enum class enableBrightEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(enableBrightEnum::SWITCH_OFF, enableBright)) etc... 

	int gainStructure = 0;
	enum class gainStructureEnum { low,medium,high };	// to compare: if(compareEnumToInt(gainStructureEnum::low, gainStructure)) etc... 

	// --- Meter Plugin Variables
	float triode1_DCMeter = 0.f;
