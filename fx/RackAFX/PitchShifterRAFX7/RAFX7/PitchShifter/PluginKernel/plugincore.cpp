...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::pitchShiftSemitones, "Pitch Shift", "semi", controlVariableType::kInt, -12.000000, 12.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&pitchShiftSemitones, boundVariableType::kInt);
	addPluginParameter(piParam);

	// --- discrete control: PhaseLock
	piParam = new PluginParameter(controlID::enablePeakPhaseLocking, "PhaseLock", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&enablePeakPhaseLocking, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);

	// --- discrete control: PeakTracking
	piParam = new PluginParameter(controlID::enablePeakTracking, "PeakTracking", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&enablePeakTracking, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);
	...
	// --- RAFX GUI attributes
	// --- controlID::pitchShiftSemitones
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::pitchShiftSemitones, auxAttribute);

	// --- controlID::enablePeakPhaseLocking
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741824);
	setParamAuxAttribute(controlID::enablePeakPhaseLocking, auxAttribute);

	// --- controlID::enablePeakTracking
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741824);
	setParamAuxAttribute(controlID::enablePeakTracking, auxAttribute);
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	// --- reset vocoders
	for(int i=0; i< NUM_VOCODER_CHANNELS; i++)
		psmVocoder[i].reset(resetInfo.sampleRate);
...
{
	PSMVocoderParameters params = psmVocoder[0].getParameters();
	params.pitchShiftSemitones = pitchShiftSemitones;
	params.enablePeakPhaseLocking = enablePeakPhaseLocking == 1;
	params.enablePeakTracking = enablePeakTracking == 1;
	
	for (int i = 0; i< NUM_VOCODER_CHANNELS; i++)
		psmVocoder[i].setParameters(params);
}
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	double ynL = psmVocoder[0].processAudioSample(xnL);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		double ynR = psmVocoder[1].processAudioSample(xnR);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::pitchShiftSemitones, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::enablePeakPhaseLocking, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::enablePeakTracking, -0.000000);
	addPreset(preset);
