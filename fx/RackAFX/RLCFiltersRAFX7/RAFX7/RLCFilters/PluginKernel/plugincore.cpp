...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	if (compareIntToEnum(filterType, filterTypeEnum::RLC_BPF))
		ynL = rlcBPF[0].processAudioSample(xnL);
	if (compareIntToEnum(filterType, filterTypeEnum::RLC_BSF))
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		if (compareIntToEnum(filterType, filterTypeEnum::RLC_LPF))
			ynR = rlcLPF[1].processAudioSample(xnR);
		else if (compareIntToEnum(filterType, filterTypeEnum::RLC_HPF))
			ynR = rlcHPF[1].processAudioSample(xnR);
		if (compareIntToEnum(filterType, filterTypeEnum::RLC_BPF))
			ynR = rlcBPF[1].processAudioSample(xnR);
		if (compareIntToEnum(filterType, filterTypeEnum::RLC_BSF))
			ynR = rlcBSF[1].processAudioSample(xnR);
