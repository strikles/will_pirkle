...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::feedback_Pct, "Feedback", "%", controlVariableType::kDouble, 0.000000, 100.000000, 50.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.00);
	...
	piParam = new PluginParameter(controlID::modDelayEffect, "Effect", "flanger,chorus,vibrato", "flanger");
	piParam->setBoundVariable(&modDelayEffect, boundVariableType::kInt);
	...
	// --- controlID::modDelayEffect
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(805306368);
	setParamAuxAttribute(controlID::modDelayEffect, auxAttribute);
...
{
	ModulatedDelayParameters params = modDelay.getParameters();
	params.lfoDepth_Pct = lfoDepth_Pct;
	params.lfoRate_Hz = lfoRate_Hz;
	params.feedback_Pct = feedback_Pct;
	params.algorithm = convertIntToEnum(modDelayEffect, modDelaylgorithm);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	return modDelay.processAudioFrame(processFrameInfo.audioInputFrame,
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::feedback_Pct, 50.000000);
	setPresetParameter(preset->presetParameters, controlID::modDelayEffect, -0.000000);
	addPreset(preset);
