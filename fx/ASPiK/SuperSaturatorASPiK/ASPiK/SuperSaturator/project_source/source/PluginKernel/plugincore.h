...
public:
	...
	// --- C++ Effect Objects: simple declaration of array of objects
	//
	TurboDistorto distortion[2];
...
private:
	...
	double inputTrim_010 = 0.0;
	double distortionControl_010 = 0.0;
	double toneControl_010 = 0.0;
	double volumeControl_010 = 0.0;

	// --- Discrete Plugin Variables 
	int engageTurbo = 0;
	enum class engageTurboEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(engageTurboEnum::SWITCH_OFF, engageTurbo)) etc... 

	int filterBypass = 0;
	enum class filterBypassEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(filterBypassEnum::SWITCH_OFF, filterBypass)) etc... 

	// --- Meter Plugin Variables
	float turboLED = 0.f;
	float filterBypassLED = 0.f;
