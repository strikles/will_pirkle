...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::filterFc_Hz, "fc", "Hz", controlVariableType::kDouble, 20.000000, 10000.000000, 1000.000000, taper::kVoltOctaveTaper);
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	// --- reset components
	leftEnvFollower.reset(resetInfo.sampleRate);
	rightEnvFollower.reset(resetInfo.sampleRate);
...
{
	EnvelopeFollowerParameters params = leftEnvFollower.getParameters();
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	double ynL = leftEnvFollower.processAudioSample(xnL);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		double xnR = processFrameInfo.audioInputFrame[1]; //< framework input sample L
		
		// --- process the audio through the right channel C++ effects object(s)
		double ynR = leftEnvFollower.processAudioSample(xnR);
