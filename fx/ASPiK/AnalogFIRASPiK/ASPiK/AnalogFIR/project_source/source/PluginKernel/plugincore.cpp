...
bool PluginCore::initPluginParameters()
{
	...
	// --- discrete control: Analog Filter
	piParam = new PluginParameter(controlID::filterType, "Analog Filter", "LPF1,HPF1,LPF2,HPF2,BPF2,BSF2", "LPF1");
...
bool PluginCore::preProcessAudioBuffers(ProcessBufferInfo& processInfo)
{
	...
	// --- update GUI parameters, CPU intensive
	updateParameters();
...
{
	AnalogFIRFilterParameters params = filter.getParameters();
	params.fc = filterFc_Hz;
	params.Q = filterQ;
	params.filterType = convertIntToEnum(filterType, analogFilter);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	// --- this is a CPU intensive update, so the update function is called from preProcessAudioBuffers()
	//     
	// --- CAUTION: linear convolution is CPU intensive
	//              Build in RELEASE mode if you experience stuttering or dropouts
	// --- do left channel (this is a MONO plugin!!)
	...
	double ynL = filter.processAudioSample(xnL);
