...
const unsigned int NUM_VOCODER_CHANNELS = 2;
...
enum controlID {
	...
	enablePeakPhaseLocking = 2,
	enablePeakTracking = 3
};
...
public:
	...
	// --- C++ Effect Objects: simple declaration of stereo array
	//
	PSMVocoder psmVocoder[NUM_VOCODER_CHANNELS];
...
private:
	...
	int enablePeakPhaseLocking = 0;
	enum class enablePeakPhaseLockingEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(enablePeakPhaseLockingEnum::SWITCH_OFF, enablePeakPhaseLocking)) etc... 

	int enablePeakTracking = 0;
	enum class enablePeakTrackingEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(enablePeakTrackingEnum::SWITCH_OFF, enablePeakTracking)) etc... 
